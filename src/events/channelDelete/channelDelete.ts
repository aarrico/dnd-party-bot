  ...
  async (channel: DMChannel | NonThreadGuildBasedChannel) => {
    ...
    try {
      ...
      if (sessions.length > 0) {
        logger.info(
          `Channel ${channel.name} (${channel.id}) was deleted. Auto-canceling ${sessions.length} session(s).`
        );
        ...
        for (const session of sessions) {
          try {
            ...
            logger.info(
              `Successfully canceled session ${session.id} (${session.name})`
            );
          } catch (error) {
      ...
      logger.error(
        `Error processing channel deletion for ${channel.id}:`,
        error
      );
    }
  }
