...
export default new Event(
  ...
  async (message: Message | PartialMessage) => {
    try {
      ...
      if (session) {
        logger.info(
          `Session message ${message.id} (${session.name}) was deleted. Auto-canceling session.`
        );
        ...
        logger.info(
          `Successfully canceled session ${session.id} due to deleted message`
        );
      }
    } catch (error) {
      // Session doesn't exist or already canceled - that's fine
      logger.debug(
        `Message ${message.id} deleted but no active session found:`,
        error
      );
